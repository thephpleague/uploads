...
class FileTest extends PHPUnit_Framework_TestCase
{
    public function setUp()
    {
        ...
        $phpunit = $this;
        \League\Uploads\FileInfo::setFactory(function ($tmpName, $name) use ($phpunit) {
    ...
    {
        $file = new \League\Uploads\File('multiple', $this->storage);
    ...
    {
        $file = new \League\Uploads\File('single', $this->storage);
    ...
    {
        $file = new \League\Uploads\File('bar', $this->storage);
    }
    ...
    public function testCallbacks()
    {
        ...
        $callbackBeforeValidate = function (\League\Uploads\FileInfoInterface $fileInfo) {
            echo 'BeforeValidate: ' . $fileInfo->getName(), PHP_EOL;
        };

        $callbackAfterValidate = function (\League\Uploads\FileInfoInterface $fileInfo) {
            echo 'AfterValidate: ' . $fileInfo->getName(), PHP_EOL;
        };

        $callbackBeforeUpload = function (\League\Uploads\FileInfoInterface $fileInfo) {
            echo 'BeforeUpload: ' . $fileInfo->getName(), PHP_EOL;
        };

        $callbackAfterUpload = function (\League\Uploads\FileInfoInterface $fileInfo) {
    ...
    {
        $file = new \League\Uploads\File('single', $this->storage);
        $file->addValidation(new \League\Uploads\Validation\Mimetype(array(
            'text/plain'
        )));
    ...
    public function testAddMultipleValidations()
    {
        ...
        $file->addValidations(array(
            new \League\Uploads\Validation\Mimetype(array(
                'text/plain'
            )),
            new \League\Uploads\Validation\Size(50) // minimum bytesize
        ));
    ...
    {
        $file = new \League\Uploads\File('bad', $this->storage);
        $this->assertFalse($file->isValid());
    }
    ...
    {
        $this->assertEquals(100, \League\Uploads\File::humanReadableToBytes('100'));
        $this->assertEquals(102400, \League\Uploads\File::humanReadableToBytes('100K'));
        $this->assertEquals(104857600, \League\Uploads\File::humanReadableToBytes('100M'));
        $this->assertEquals(107374182400, \League\Uploads\File::humanReadableToBytes('100G'));
        $this->assertEquals(100, \League\Uploads\File::humanReadableToBytes('100F')); // <-- Unrecognized. Assume bytes.
    }
}
